...
#     minus 25dB. (25 dB is n line with the standard setting to detect silence
...
def get_files():
    path = os.path.join(input_dir, '*.wav')
    files = glob(path)
...
if __name__ == "__main__":
    ...
    # if len(sys.argv) != 3:
    #    print("python syllables.py <input_dir> <output_dir>")
    #    sys.exit()
    ...
    #    if 'PEC_059_ses01_Wordcolor' not in fil:
            print(len(os.listdir(input_dir)))
            file = os.path.join(input_dir, fil)
            print(file)
            speechrate_dictionary = speech_rate(file)
            datalist.append(speechrate_dictionary)
    df = pd.DataFrame(datalist)
    out_path = os.path.join(output_dir, 'speechrate_data.csv')
    df.to_csv(out_path)
