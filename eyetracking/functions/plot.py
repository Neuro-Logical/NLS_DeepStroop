...
def plot_raw(df_trial, col_timestamp, col_pos_x, col_pos_y, save_plots=False, save_path=None):
    _, ax0 = plt.subplots()
    ...
    plt.plot( df_trial[col_pos_x], df_trial[col_pos_y], 'b')
...
def plot_saccades( df_trial, col_timestamp, col_pos_x_left, col_pos_y_left, col_vel_left, col_acel_left, col_saccade_left, threshold_fixVel, threshold_fixAcc, save_plots=False, save_path=None):
...
def plot_saccades_blinks(df_trial, col_timestamp, col_pos_x_left, col_pos_y_left, col_vel_left, col_acel_left, col_saccade_left, col_blink_left, threshold_fixVel, threshold_fixAcc, save_plots=False, save_path=None):
    ...
    if len(startStop_blink_left) > 0:
        plt.plot([df_trial.loc[startStop_blink_left[0][0], col_timestamp], df_trial.loc[startStop_blink_left[0][1], col_timestamp]], [0,0], 'g', linewidth=5, label='blink')
        for start_blink, end_blink in startStop_blink_left[1:]:
            plt.plot([df_trial.loc[start_blink, col_timestamp], df_trial.loc[end_blink, col_timestamp]], [0,0], 'g', linewidth=5)
    plt.plot(df_trial.loc[startStop_saccade_left[0][0]:startStop_saccade_left[0][1], col_timestamp], df_trial.loc[startStop_saccade_left[0][0]:startStop_saccade_left[0][1],col_pos_x_left], 'r', label='saccade', linewidth=3, alpha=0.7)
...
def plot_wordTokens_correct( data_wordAlign, col_time, col_token, col_wordIndex, save_plots=False, save_path=None):
    plt.figure()

    text_replace = { '▁RE': '_RED', '▁B': '_BLUE', '▁G': '_GREEN'}
    for _, row in data_wordAlign.iterrows():
        try:
            text_annotate = '_' + text_replace[row[col_token]]
        except:
            text_annotate = '_' + row[col_token][:3]
        
        if np.isnan( row[col_wordIndex]):
            plt.plot(row[col_time], -1, 'ro')
            plt.annotate(text_annotate, [row[col_time], -1])
        else:
            plt.plot(row[col_time], row[col_wordIndex], 'go')
            plt.annotate(text_annotate, [row[col_time], row[col_wordIndex]])
...
def plot_alignedGazeAudio( data_sac, data_fix, data_blk, data_wordAlign, data_audio, time_audio, col_time, title='', save_plots=False, save_path=None, annotate_tokens=True):
    plt.figure(figsize=(16,9))
    ax0 = plt.subplot(2,1,1)
    _plot_gazeWord( data_fix, data_blk, data_sac, df_event=data_wordAlign, df_event_colTime=col_time, df_event_colEvent='word_word')
    plt.title(title+' EYE GAZE')
    plt.ylabel('Word Index')

    ax2 = plt.subplot(2,1,2,sharex=ax0)
    plt.plot(time_audio,data_audio)
    plt.plot(data_wordAlign[col_time], data_wordAlign['word_word']-0.05, 'y^')
    if annotate_tokens:
        text_replace = { '▁RE': '_R', '▁B': '_B', '▁G': '_G', 'red': '_R', 'blu': '_B', 'gre': '_G'}
        for i, (text, pos_x, pos_y, goodAlign) in enumerate(zip( data_wordAlign['word'], data_wordAlign[col_time], data_wordAlign['word_word'], data_wordAlign['alignment'])):
            pos_y_adj = pos_y - 6000 - (1000 * ((i+1) % 2))
            try:
                text_annotate = '_' + text_replace[text]
            except:
                text_annotate = '_' + text[:3]
            if goodAlign:
                plt.plot([pos_x, pos_x], [pos_y, pos_y_adj], 'y')
            else:
                plt.plot([pos_x, pos_x], [pos_y, pos_y_adj], 'r')
            plt.annotate(  text_annotate, (pos_x, pos_y_adj))
    ...
    # plt.figure()
    # plt.plot(time_audio,data_audio)
    # plt.plot(data_wordAlign['Time'], data_wordAlign['word_word']-0.05, 'y^')
    # if annotate_tokens:
    #     text_replace = { '▁RE': '_RED', '▁B': '_BLUE', '▁G': '_GREEN'}
    #     for i, (text, pos_x, pos_y) in enumerate(zip( data_wordAlign['Token'], data_wordAlign['Time'], data_wordAlign['word_word'])):
    #         pos_y_adj = pos_y - 6000 - (1000 * ((i+1) % 2))
    #         try:
    #             text_annotate = text_replace[text]
    #         except:
    #             text_annotate = text
    #         plt.plot([pos_x, pos_x], [pos_y, pos_y_adj], 'y')
    #         plt.annotate(  text_annotate, (pos_x, pos_y_adj))
...
def plot_wordCorrect_value( data_sac, data_fix, data_blk, data_wordAlign, col_time, col_value, col_color=None, save_plots=False, save_path=None):
    if col_value not in data_wordAlign.keys():
        return
    plt.figure(figsize=(16,9))
    ax0 = plt.subplot(2,1,1)
    _plot_gazeWord( data_fix, data_blk, data_sac, df_event=data_wordAlign, df_event_colTime=col_time, df_event_colEvent='word_word')
    ...
    if col_color is not None:
        for _, row in data_wordAlign.iterrows():
            try:
                plt.plot( [row[col_time],row[col_time]], [0,row[col_value]], 'o-', color=row[col_color][0])
            except:
