...
def get_saccades( df_data, column_positionX, column_positionY, column_velocity, column_acceleration, column_blink, threshold_fixDist=20, threshold_fixVel=25, threshold_fixAcc=3000):
    '''Identifies blink events, and identifies each individual start/stop index
    ...
    sac_startStop = get_indexStartStop(df_data[column_velocity].abs() > threshold_fixVel)

    # Create the final blink output array, initialized to all false
    sac_return = pd.Series(False, index=df_data.index)
    for start, end in sac_startStop:
...
def get_stimuliFixation(df_fixation, name_stimuli, col_fix_time_start, col_fix_time_end, col_fix_pos_x, col_fix_pos_y, col_fix_duration, col_gaze='gaze', col_gaze_line='gazeLine', col_gaze_word='gazeWord', col_gaze_text='gazeText', col_gaze_color='gazeColor', col_focus='focus', notes=None):
    ...
    if clusters_initial_x is None:
        print('\t\tWARNING: GAZE: No stimuli defined for ', name_stimuli)
    ...
    if len(df_fixation_use) == 0:
        print('\t\tWARNING: GAZE: No usable fixations found')
    ...
    # plt.plot(df_fixation_use.iloc[identified_lines[0][1]-1:identified_lines[1][0]+1][col_fix_time_start], df_fixation_use.iloc[identified_lines[0][1]-1:identified_lines[1][0]+1][col_fix_pos_x], 'gre', label='newline')
    # for i in range(1, len(identified_lines)-1):
    #     plt.plot(df_fixation_use.iloc[identified_lines[i][1]-1:identified_lines[i+1][0]+1][col_fix_time_start], df_fixation_use.iloc[identified_lines[i][1]-1:identified_lines[i+1][0]+1][col_fix_pos_x], 'gre')
    ...
    # plt.plot(plot_initial_x, plot_initial_y, 'c.', markersize=12, alpha=0.2, label='"True" loc')
    # plt.plot(df_fixation_use[col_fix_pos_x], df_fixation_use[col_fix_pos_y], 'b.', markersize=8, label='fixation')
    # plt.plot(df_fixation_use.iloc[identified_lines[0][1]-1:identified_lines[1][0]+1][col_fix_pos_x], df_fixation_use.iloc[identified_lines[0][1]-1:identified_lines[1][0]+1][col_fix_pos_y], 'gre', label='newline')
    # for i in range(1, len(identified_lines)-1):
    #     plt.plot(df_fixation_use.iloc[identified_lines[i][1]-1:identified_lines[i+1][0]+1][col_fix_pos_x], df_fixation_use.iloc[identified_lines[i][1]-1:identified_lines[i+1][0]+1][col_fix_pos_y], 'gre')
    # plt.xlim((0,1920))
    # plt.ylim((1080, 0)) # Invert Y axis
    # plt.legend()
    # plt.show()
    ...
    if len(identified_lines) != num_lines:
        print('\t\tWARNING: GAZE: Incorrect number of lines identified')
        return df_fixation, None, (num_words, num_lines)
    ...
    for gaze_line, (index_start, index_stop) in enumerate(identified_lines):
        ...
        if pos_fix.shape[0] < num_words:
            print('\t\tWARNING: GAZE: Incorrect number of words identified')
            return df_fixation, None, (num_words, num_lines)
        try:
            ...
            print('\tERROR: GAZE: Cannot find KMeans Clusters\t', )
            print(e)
            return df_fixation, None, (num_words, num_lines)
...
def clean_wordTokens(df_wordTokens, col_token, col_repeat, col_time, col_delay, col_inVocab, repeated_minTime=100):
    ...
    Returns:
        df_wordCorrect dataframe containing word tokens without repeats.
    '''
    ...
    df_wordCorrect = df_wordTokens.copy().drop(df_wordTokens[(df_wordTokens[col_delay] < repeated_minTime) & df_wordTokens[col_repeat]].index)
    # Clean out of vocabulary words
    df_wordCorrect = df_wordCorrect.drop(df_wordCorrect[ ~df_wordCorrect[col_inVocab]].index)

    return df_wordCorrect



def get_wordCorrectSequence( df_wordCorrect, df_fixation, stimuli, col_time, col_token, col_fixationTime, col_focus, col_fixationGazeIndex, col_wordIndex):
    '''Determine whether the word tokens are correct and occur in the expected sequence. 
    
    Args:
        df_wordCorrect (pandas dataframe): Dataframe containing cleaned and verified word tokens.
        df_fixation (pandas dataframe):    Pandas dataframe containing data about individual fixations
        stimuli (list):                    Stimuli descriptions provided by :py:func:`~eyetracking.functions.process.get_stimuliDescriptions`
        col_time (str):                    Column header in `df_wordCorrect` designating the word start timestamp.
        col_token (str):                   Column header in `df_wordCorrect` deisgnating the word token.
        col_fixationTime (str):            Column header in `df_fixation` designating the timestamp (ms) of each fixation start.
        col_focus (str):                   Column header in `df_fixation` designating the focus state.
        col_fixationGazeIndex (str):       Column header in `df_fixation` designating the word (index) the subject is looking at.
        col_wordIndex                      Column header in `df_wordCorrect` designating where to place the index of correct words. Incorrect words will be left as Nan. 

    Returns:
        df_wordCorrect with correct tokens identified. Incorrecte tokens will have nan values in `col_wordIndex`
    '''
    df_wordCorrect = get_wordCorrectSequence_onlyAudio( df_wordCorrect, stimuli, col_token, col_wordIndex)
    if df_wordCorrect['correct'].mean() < 0.66:
        if df_wordCorrect[col_token].values[-1] != stimuli[-1][0]:
            # Sometimes the last token is cut off. Try while omitting the final token
            df_wordCorrect = get_wordCorrectSequence_onlyAudio( df_wordCorrect, stimuli[:-1], col_token, col_wordIndex)
        
        if df_wordCorrect['correct'].mean() < 0.66:
            # If no words are found using the audio, the trial progress might be too irregular. Trust the eye-aligned assignment
            df_wordCorrect = get_wordCorrectSequence_primaryEye( df_wordCorrect, df_fixation, stimuli, col_time, col_token, col_fixationTime, col_focus, col_fixationGazeIndex, col_wordIndex)
    
    if col_focus in df_fixation.columns:
        df_fixation_use = df_fixation.loc[ df_fixation[col_focus] == True, :]
    else:
        df_fixation_use = df_fixation

    df_wordCorrect['alignment'] = False
    for index_stimuli in range(len(stimuli)):
        first_fixation = df_fixation_use.loc[ (df_fixation_use[col_fixationGazeIndex] == index_stimuli), col_fixationTime]
        if len(first_fixation) > 0:
            first_fixation = first_fixation.values[0]
        else:
            continue

        time_spoken = df_wordCorrect.loc[ df_wordCorrect[col_wordIndex] == index_stimuli , col_time]
        if (len(time_spoken) > 0):
            time_spoken = time_spoken.values[0]
        else:
            continue

        if time_spoken > first_fixation:
            df_wordCorrect.loc[ df_wordCorrect[col_wordIndex] == index_stimuli , 'alignment']     = True
        else:
            df_wordCorrect.loc[ df_wordCorrect[col_wordIndex] == index_stimuli , 'alignment']     = False
    
    return df_wordCorrect



def get_wordCorrectSequence_primaryEye( df_wordCorrect, df_fixation, stimuli, col_time, col_token, col_fixationTime, col_focus, col_fixationGazeIndex, col_wordIndex):
    '''Determine whether the word tokens are correct and occur in the expected sequence. 
    
    Note:
        This function does not work as well on stroop tasks due to the many occurances of the same word at very close timestamps.
        Often eyes move faster than speech, and if eyes lead too much there is ambiguity in word assignment.
    ...
    Returns:
        df_wordCorrect with correct tokens identified. Incorrecte tokens will have nan values in `col_wordIndex`
    '''
    df_wordCorrect['correct'] = False
    
    time_stimuli  = [-1] * len(stimuli)
    # since sometimes the arbitrary "Focus" start can omit the first work, we automatically populate the first word
    if (df_wordCorrect[col_token] == stimuli[0,0]).sum() == 0:
        # No correct tokens exist
        df_wordCorrect[col_wordIndex] = np.nan
        return df_wordCorrect
    time_stimuli[0] = df_wordCorrect.loc[ df_wordCorrect[col_token] == stimuli[0,0], col_time].values[0]
    ...
    for index_stimuli in range(1,len(stimuli)):
        first_fixation = df_fixation_use.loc[ (df_fixation_use[col_fixationGazeIndex] == index_stimuli), :]
        if len(first_fixation) > 0:
            first_fixation = first_fixation.iloc[0,:]
        else:
            continue

        target = stimuli[index_stimuli,0]
        word_possible = df_wordCorrect[ (df_wordCorrect[col_time] >= first_fixation[col_fixationTime]) & (df_wordCorrect[col_token] == target)]
        if len(word_possible) > 0:
            time_spoken = word_possible.loc[:,col_time].values[0]
            # If this word was already assigned, remove the previous assignment
            while time_spoken in time_stimuli:
                time_stimuli[ time_stimuli.index(time_spoken) ] = -1
            time_stimuli[index_stimuli] = time_spoken

    # Previous stimuli should not come before this one
    for i in range(len(time_stimuli)-1,0,-1): # Iterate backwards through the list
        if time_stimuli[i] < 0:
            continue
        for j in range(i):                    # Compare all preceeding indexes
            if time_stimuli[j] > time_stimuli[i]:
                time_stimuli[j] = -1
    
    if time_stimuli[-1] < max(time_stimuli[:-1]):
        time_stimuli[-1] = -1
    
    df_wordCorrect[col_wordIndex] = np.nan
    for index in range(len(stimuli)):
        if time_stimuli[index] >= 0:
            df_wordCorrect.loc[ df_wordCorrect[col_time] == time_stimuli[index], 'correct']     = True
            df_wordCorrect.loc[ df_wordCorrect[col_time] == time_stimuli[index], col_wordIndex] = index
    
    return df_wordCorrect



def get_wordCorrectSequence_onlyAudio( df_wordCorrect, stimuli, col_token, col_wordIndex):
    '''Determine whether the word tokens are correct and occur in the expected sequence. Uses only the audio transcription, analyzing the order of the spoken tokens forwards and backwards.
    ...
    df_wordCorrect['correct_forward'] = np.nan
    index_correct = df_wordCorrect.columns.get_loc('correct_forward')
    index_word = 0
    for i_truth, truth in enumerate(stimuli):
        while index_word < len(df_wordCorrect):
            tokenFirst = df_wordCorrect.iloc[index_word][col_token]
            if tokenFirst.startswith( truth[0]):
                df_wordCorrect.iloc[index_word,index_correct] = i_truth
                break
            if truth[1].startswith(tokenFirst):
                df_wordCorrect.iloc[index_word,index_correct] = -1 # Negative one indicates they said word not color
            index_word += 1
        index_word += 1

    # Identify correctly spoken words, by interating backward through the tokens
    df_wordCorrect['correct_backward'] = np.nan
    index_correct = df_wordCorrect.columns.get_loc('correct_backward')
    index_word = len(df_wordCorrect) - 1
    for i_truth, truth in reversed(list(enumerate(stimuli))):
        while index_word >= 0:
            ...
            if tokenFirst.startswith(truth[0]):
                ...
                break
            if truth[1].startswith(tokenFirst):
                df_wordCorrect.iloc[index_word,index_correct] = -1
                break
            index_word -= 1
        index_word -= 1

    # Tokens which match the forward and backwards progression are confidently correct.
    index_matching = df_wordCorrect[ df_wordCorrect['correct_forward'] == df_wordCorrect['correct_backward']].index
    df_wordCorrect[col_wordIndex] = np.nan
    df_wordCorrect.loc[index_matching,col_wordIndex] = df_wordCorrect.loc[index_matching,'correct_forward']
    df_wordCorrect.loc[index_matching,'correct']     = True
...
def get_tokenVocabulary(name_trial):
    ...
    if ('-naming' in trial) or ('stroop' in trial) or ('colors_preliminary' in trial):
        stimuli.append(['red'])
        stimuli.append(['blu'])
        stimuli.append(['gre'])
...
def get_stimuliDescriptions(name_trial):
    ...
    trial = name_trial.strip().lower()
    if trial in ['color-naming','word-color-naming','stroop','strooponlycolors','colors_preliminary2']:
        stimuli.append(['red','blu']) # line 0 word 0
        stimuli.append(['blu','red']) # line 0 word 1
        stimuli.append(['red','gre']) # line 0 word 2
        stimuli.append(['gre','blu']) # line 0 word 3
        stimuli.append(['blu','gre']) # line 1 word 0
        stimuli.append(['gre','blu']) # line 1 word 1
        stimuli.append(['blu','red']) # line 1 word 2
        stimuli.append(['blu','gre']) # line 1 word 3
        stimuli.append(['gre','red']) # line 2 word 0
        stimuli.append(['red','gre']) # line 2 word 1
        stimuli.append(['gre','blu']) # line 2 word 2
        stimuli.append(['red','blu']) # line 2 word 3
        stimuli.append(['red','blu']) # line 3 word 0
        stimuli.append(['blu','red']) # line 3 word 1
        stimuli.append(['blu','gre']) # line 3 word 2
        stimuli.append(['gre','red']) # line 3 word 3
        stimuli.append(['blu','red']) # line 4 word 0
        stimuli.append(['red','gre']) # line 4 word 1
        stimuli.append(['gre','red']) # line 4 word 2
        stimuli.append(['red','gre']) # line 4 word 3
        stimuli.append(['red','gre']) # line 5 word 0
        stimuli.append(['gre','blu']) # line 5 word 1
        stimuli.append(['red','blu']) # line 5 word 2
        stimuli.append(['blu','red']) # line 5 word 3

    elif trial in ['word-naming','strooponlytext','colors_preliminary1']:
        stimuli.append(['blu','bla']) # line 0 word 0
        stimuli.append(['gre','bla']) # line 0 word 1
        stimuli.append(['blu','bla']) # line 0 word 2
        stimuli.append(['gre','bla']) # line 0 word 3
        stimuli.append(['red','bla']) # line 1 word 0
        stimuli.append(['blu','bla']) # line 1 word 1
        stimuli.append(['gre','bla']) # line 1 word 2
        stimuli.append(['red','bla']) # line 1 word 3
        stimuli.append(['gre','bla']) # line 2 word 0
        stimuli.append(['red','bla']) # line 2 word 1
        stimuli.append(['red','bla']) # line 2 word 2
        stimuli.append(['blu','bla']) # line 2 word 3
        stimuli.append(['red','bla']) # line 3 word 0
        stimuli.append(['blu','bla']) # line 3 word 1
        stimuli.append(['gre','bla']) # line 3 word 2
        stimuli.append(['red','bla']) # line 3 word 3
        stimuli.append(['gre','bla']) # line 4 word 0
        stimuli.append(['gre','bla']) # line 4 word 1
        stimuli.append(['red','bla']) # line 4 word 2
        stimuli.append(['gre','bla']) # line 4 word 3
        stimuli.append(['blu','bla']) # line 5 word 0
        stimuli.append(['red','bla']) # line 5 word 1
        stimuli.append(['blu','bla']) # line 5 word 2
        stimuli.append(['blu','bla']) # line 5 word 3
...
def remove_outliers( data, threshold_min, threshold_max):
    ...
    with np.errstate(invalid='ignore'):
        index_min = (data < threshold_min).nonzero()[0]
        index_max = (threshold_max < data).nonzero()[0]
